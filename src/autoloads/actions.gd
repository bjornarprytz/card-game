...
func initialize_game_state(game_state: GameState, initial_conditions: Variant) -> void:
	...
	for card_name in initial_conditions["deck"]:
		_resolve_operation_tree(Keywords.create_operation_tree("create_card", [game_state, card_name, game_state.draw_pile]))
	for card_name in initial_conditions["discard_pile"]:
		_resolve_operation_tree(Keywords.create_operation_tree("create_card", [game_state, card_name, game_state.discard_pile]))
	for card_name in initial_conditions["hand"]:
		_resolve_operation_tree(Keywords.create_operation_tree("create_card", [game_state, card_name, game_state.hand]))
	for creature_name in initial_conditions["battlefield"]:
		_resolve_operation_tree(Keywords.create_operation_tree("create_creature", [game_state, creature_name, game_state.battlefield]))

func play_card(context: PlayCardContext) -> bool: # TODO: Move this wholly to GameLoop/GameAction
	...
	for effect in effects:
		...
		if effect.evaluate_condition(context):
			...
			_resolve_operation_tree(Keywords.create_operation_tree(effect.keyword, args))

	return true

func _resolve_operation_tree(operation_tree: KeywordNode):
