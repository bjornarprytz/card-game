...
func add_modifier(modifier: Modifier, targets: Array[Atom], scope: Scope) -> Array[Operation]:
    return [NoopOperation.new()]

func fight(c1: Creature, c2: Creature) -> Array[KeywordNode]:
    var sub_nodes: Array[KeywordNode] = []
    
    sub_nodes.append(create_operation_tree("attack", [c2, c1]))
    sub_nodes.append(create_operation_tree("attack", [c1, c2]))
...
func attack(source: Creature, target: Creature) -> Array[KeywordNode]:
    ...
    sub_nodes.append(create_operation_tree("deal_damage", [target, source.attack]))
