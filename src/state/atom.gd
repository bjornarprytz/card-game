...
var current_zone: Zone:
    set(value):
        if (current_zone != value):
            current_zone = value
            _set_state("current_zone", current_zone)

    get:
        return _get_state("current_zone", null)
...
func _init_properties(properties: Dictionary[String, Variant]) -> void:
    ...
    for prop in properties.keys():
        _set_state(prop, properties[prop])
...
func _get_state(key: String, default_value: Variant = null) -> Variant:
    key = key.to_lower()
    if (_state.has(key)):
        return _state[key]
...
func _remove_state(key: String) -> bool:
    key = key.to_lower()
    if (!_state.has(key)):
...
func _set_state(key: String, value: Variant) -> bool:
    key = _clean_key(key)
