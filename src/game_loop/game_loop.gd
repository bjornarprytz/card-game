...
func try_prompt_response(prompt_response: PromptResponse) -> bool:
    ...
    if (not pending_prompt.try_resolve(prompt_response)):
        push_warning("Failed to resolve prompt response.")
        return false
...
func _resolve_next_keyword():
    ...
    if (keyword_node is PromptNode):
        _enqueue_prompt(keyword_node.create_prompt(current_effect_block.context))
        return
