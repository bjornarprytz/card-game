...
static func from_variant(param: Variant) -> ParameterProto:
    ...
    if param is String:
        ...
        if param.begins_with("\"") and param.ends_with("\"") and param.length() >= 2:
            ...
            if (parameter._expression is not SimpleExpression):
                push_warning("Parsed complex expression for an effect: %s" % param)
    else:
...
func _to_string() -> String:
    ...
    elif _expression != null:
        return _expression.expression_string
    else:
        return "N/A"
