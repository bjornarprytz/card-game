...
## -------------
## 1. TARGET ACCESS:
##    - t0, t1, t2, etc.: Access targets by index from the context's chosen_targets array
...
static func t(context: Context, index: int) -> Variant:
	if index >= 0 and index < context.chosen_targets.size():
		return context.chosen_targets[index]
