...
func execute() -> Array[StateChange]:
    ...
    if atom.current_zone != null:
        ...
        operations.append(SetState.new(current_zone, "atoms", current_zone_new_atoms))

    operations.append(SetState.new(atom, "current_zone", zone))

    if zone != null:
        var new_zone_new_atoms = zone.atoms.duplicate()
        new_zone_new_atoms.append(atom)
        operations.append(SetState.new(zone, "atoms", new_zone_new_atoms))
